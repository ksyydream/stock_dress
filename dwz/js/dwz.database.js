	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				var addTitle = $table.attr('addTitle') || "款式颜色";
				if (addTitle) {
					var $rowaddTitle = $('<a herf="javascript:void(0);">删除所有 '+addTitle+' 颜色的尺码</a>').insertBefore($table).find("a");
					$addBut.click(function(){
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					case 'enum':
						...
						break;
					case 'hidden':
						html = '<input type="hidden" name="'+field.name+'" value="'+field.defaultVal+'"  '+attrFrag+'/>';
						break;
					case 'read_text':
						html = '<input type="text" readonly="readonly" name="'+field.name+'" value="'+field.defaultVal+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+'/>';
						break;
					case 'date':
