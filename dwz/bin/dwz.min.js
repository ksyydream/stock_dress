...
initSuffix($tbody);});}
    ...
    if (addTitle !="款式颜色") {
        var $rowaddTitle = $('<a herf="javascript:void(0);" class="am-btn">删除所有 '+addTitle+' 颜色的尺码</a>').insertBefore($table);
        $rowaddTitle.click(function(){
            alertMsg.confirm("是否确认删除?", {
                okCall: function(){
                    $rowaddTitle.prev().prev().remove();
                    $rowaddTitle.prev().remove();
                    $table.remove();
                    $rowaddTitle.remove();
                }
            });
